...
| `Send(ctx, chatID, replyTo, text, sendType...)` | 發訊息；`sendType` 可選 `TypeMarkdown` / `TypeHTML` |
...
| `SendInput(ctx, chatID, replyTo, text, sendType...)` | `ForceReply` prompt |
| `SendSelect(ctx, chatID, replyTo, text, items, sendType...)` | inline keyboard single-select |
| `SendMultiSelect(ctx, chatID, replyTo, text, items, sendType...)` | inline keyboard multi-select |
| `SendStatus(ctx, chatID, replyTo, text, emoji...)` | per-chat 單一「思考中」訊息；首次加 reaction（預設 🤔，可自訂）；debounce 1s |
...
bot.Send(ctx, chatID, 0, "*hello*", telegram.TypeMarkdown)
